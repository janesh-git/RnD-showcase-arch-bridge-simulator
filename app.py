...
# Draw loads (Using your fixed, smaller arrows)
...
with col1:
    ...
    st.markdown("To find the precise shape of the thrust line, we calculate the outward Horizontal Thrust ($H$) required to keep the crown hinge at zero bending moment.")
    
    st.latex(rf"H = \frac{{M_{{crown}}}}{{\text{{Rise}}}}")
    st.latex(rf"H = \frac{{{moment_at_crown:.2f}}}{{{rise:.2f}}} = {H:.2f} \text{{ units}}")
    
    st.markdown(f"**Current Horizontal Thrust:** **{H:.2f}**")
